...
function getLocationFromVF(mmsi, cb) {
  ...
  request(options, function (error, response, html) {
    if (
      ...
      const position = $(
        ".vfix-top:nth-of-type(1) .tparams tr:nth-of-type(9) .v3"
      ).text();
      ...
      const lat_lon = $(
        ".vfix-top:nth-of-type(1) .tparams tr:nth-of-type(10) .v3"
      ).text();
      ...
      const latitude =
        splitted[0].indexOf("N") === -1
          ? parseFloat(splitted[0]) * -1
          : parseFloat(splitted[0]);
      const longitude =
        splitted[1].indexOf("E") === -1
          ? parseFloat(splitted[1]) * -1
          : parseFloat(splitted[1]);

      const timestamp = new Date(
        $("#lastrep").attr("data-title")
      ).toISOString();
...
function getLocationFromMT(mmsi, cb) {
  ...
  request(options, function (error, response, html) {
    if ((!error && response.statusCode == 200) || response.statusCode == 403) {
      ...
      secondRequestHeaders["Vessel-Image"] =
        "007fb60815c6558c472a846479502b668e08";
        ...
        function (error, response, html) {
          if (
            ...
            if (parsed.totalCount > 0) {
              ...
              const timestamp = new Date(
                parsed.data[0].LAST_POS * 1000
              ).toISOString();
              const unixtime =
                new Date(parsed.data[0].LAST_POS * 1000).getTime() / 1000;
              console.log(123);
